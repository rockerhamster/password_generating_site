...
function myFunction() {
    ...
    text=compress(SHA256(password(x,y.toLowerCase())));
...
function compress(str){
    ...
    for(i=0;i<str.length-1;i=i+3){
        array[i/3]=(str[i].charCodeAt(0)+str[i+1].charCodeAt(0)+str[i+2].charCodeAt(0))%127;
        if(array[i/3]<33)
        {
            array[i/3]+=33;
        }
        else if(array[i/3]<=90&&array[i/3]>=65)
        {
            if(j%2==0)
            {array[i/3]+=32;}
            j++;
        ...
        {
            temp+=String.fromCharCode(array[i/3]);
        }
    }
    return temp;
}

function password(str, str2) {
    let i=0;
    if(str.length<=str2.length)
    {
    let strlength=str2.length-str.length;
    while (i < strlength) {
        str = str +'a';
        i++;
    }
    ...
    {
    let strlength=str.length-str2.length;
    while (i < strlength) {
        str2 = str2 +'a';
        i++;
    }
    ...
    let finpass="";
    while (i < str2.length) {
        finpass+=String.fromCharCode(( str[i].charCodeAt(0) + str2[i].charCodeAt(0) )/2);
        i++;
    }
    return finpass;
}
...
//console.log(SHA256(password("dssdvdv","dcsdcd")));
//console.log(compress(SHA256(password("efgg3rg4gre","3gferg24rg4rg4"))));
//console.log(password("wefq34er","34rk"));
